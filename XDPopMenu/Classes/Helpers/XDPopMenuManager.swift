...
final public class XDPopMenuManager: NSObject {
    
    // MARK: - Properties
    
    /// Default manager singleton.
    public static let `default` = XDPopMenuManager()
    
    /// Reference to the pop menu view controller.
    private var popMenu: XDPopMenuVC!
    
    /// Reference to the pop menu delegate instance.
    public weak var popMenuDelegate: XDPopMenuVCDelegate? {
        didSet {
    ...
    public init(appearance: XDPopMenuAppearance = XDPopMenuAppearance()) {
...
extension XDPopMenuManager {
    ...
    public func present(sourceView: AnyObject? = nil, on viewController: UIViewController? = nil, animated: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if let presentOn = viewController {
            ...
            if let topViewController = XDPopMenuManager.getTopViewControllerInWindow() {
